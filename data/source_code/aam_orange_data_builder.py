...
def AamCreateOrangeData(model, out_tabfile):
    ...
    # subject_tags = [model[k].subject_id for k in range(len(model))]
    # subject_ids = np.unique(subject_tags)
    # for s in subject_ids:
    #     if subject_tags.count(s)<2: print(f'Removing Subject {s} which has only one sample')
    # subject_ids_selection = [s for s in subject_ids if subject_tags.count(s)>1]
    
    with open(out_tabfile, 'w') as f:
        f.write('mS#name\t' + '\t'.join(map('{}'.format, itertools.chain(sd, td))) + '\tD#subject' + '\tD#emotion' + '\n')
