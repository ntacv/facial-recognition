...
#        s0 = self.pcaShapeGetS(0)
    ...
    def _computeMeanShapeAlignedTexture(self, idx, dst=None):
        ...
        # Using skimage.transform.PiecewiseAffineTransform class as a temporary fix for https://github.com/scikit-image/scikit-image/issues/6264
        # tform_piecewise = skimage.transform.estimate_transform('piecewise-affine', src=s0, dst=tform_global.inverse(self[idx].points))
        tform_piecewise = skimage.transform.PiecewiseAffineTransform()
        tform_piecewise.estimate(src=s0, dst=tform_global.inverse(self[idx].points))
...
#        s0 = self.pcaShapeGetS(0, map_to_a0=True)
        ...
        tform_piecewise = skimage.transform.PiecewiseAffineTransform()
        tform_piecewise.estimate(src=shape_rec, dst=s0)
